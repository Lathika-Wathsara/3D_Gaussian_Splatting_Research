...
import wandb
from utils.blob_detection import detect_blobs_dog   # Code by lathika
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, log_to_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            # Code by lathika
            if iteration%100==0:
                print(f"radii_len = {len(radii)}, total_wanted_points = {torch.sum(~unwanted_gauss_filter)} , unwanted_neg_points = {torch.sum(unwanted_gauss_filter)}")
