...
# Code by lathika
def sigmoid_conditioned(x,isPositive):
    isPositive_f = isPositive.to(torch.float32)  # Convert boolean tensor in to float with 0 or 1 to multiply
    sig_x = torch.sigmoid(x)
    sig_conditioned = sig_x*isPositive_f + (-sig_x)*(1-isPositive_f)
    return sig_conditioned
