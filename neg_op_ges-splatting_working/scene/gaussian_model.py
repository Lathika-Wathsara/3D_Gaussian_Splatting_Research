...
import torch
import numpy as np
from utils.general_utils import sigmoid_conditioned, inverse_sigmoid, get_expon_lr_func, build_rotation # sigmoid_conditioned id code by lathika
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self._isPositive = torch.empty(0)   # Code by lathika
    ...
    def capture(self):
        return (
            ...
            self._rotation,
            self._opacity,
            self._isPositive,   # Code by lathika
            self.max_radii2D,
    ...
    def save_ply(self, path):   # Concern about how the neg ops will be shown in the web, after their rasterization
        ...
        # Note we might need to edit the viewer code to configure the behaviour with the negetive opacities.
        dtype_full = [(attribute,'b') if (attribute == 'isPositive') else (attribute, 'f4') for attribute in self.construct_list_of_attributes()]
    ...
    def load_ply(self, path):
        ...
        # Code by lathika
        isPositive = np.asarray(plydata.elements[0]["isPositive"])[..., np.newaxis] # Here we are extracting the isPositive condition (if we are initializing from a half trained ply output).
                                                                                    # See __init__.py
        ...
        self._isPositive = torch.tensor(isPositive, dtype=torch.bool, device = "cuda")  # Code by lathika
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_isPositive): # Code by lathika added "new_isPositive"
        d = {"xyz": new_xyz,
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        # Code by lathika
        prune_mask = torch.logical_and(self.get_opacity < min_opacity, self.get_opacity > -min_opacity).squeeze()
