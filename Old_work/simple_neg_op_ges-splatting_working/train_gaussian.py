...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, log_to_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Code by lathika - iteration >=2000 and iteration%1000==0
        if iteration <=12000 and iteration >=1000 and iteration%1000==0: # Changed for testing   # After 1000 iterations, neg gaussians are added with 500 intervals
        ...
        if iteration == it_1 or iteration == it_2 or  iteration == it_3:
            ...
            cv2.imwrite(f"/home/lathika/Workspace/test/Dump/Dump_img/2_with_neg_test_3/output{iteration}.jpg", image_cpu)
            cv2.imwrite(f"/home/lathika/Workspace/test/Dump/Dump_img/2_with_neg_test_3/output_gt{iteration}.jpg", gt_cpu)
        ...
        with torch.no_grad():
            ...
            if iteration%1000==0:
                ...
                if iteration >= 3000:
                    pos_s = gaussians.get_scaling[gaussians._isPositive.squeeze()]
                    pos_s_mean = torch.mean(pos_s.float(), dim=0)
                    pos_s_median = torch.median(pos_s.float(),dim=0).values
                    neg_s = gaussians.get_scaling[~gaussians._isPositive.squeeze()]
                    neg_s_mean = torch.mean(neg_s.float(), dim=0)
                    neg_s_median = torch.median(neg_s.float(),dim=0).values
                    print(f" pos_s_mean  = { pos_s_mean }, pos_s_median = {pos_s_median}, neg_s_mean = {neg_s_mean}, neg_s_median = {neg_s_median}")
                print(f"at iter {iteration} - neg_gaus = {(~gaussians._isPositive).sum()} , pos_gaus = {(gaussians._isPositive).sum()}")
            ...
            #add_neg_gauss_flage = False # Code by lathika to stop running the part below
            if add_neg_gauss_flage:
                if randint(0,20)==0:#iteration % 30 == 0:     # This step is taken to reduce the number of images that will be processed for blob detection
                                            # Every image with an interval of 30, will be selected. (We can change this)
                    print(f"\n Going through neg gaus initialization at {iteration}")  # Test
                    print(f"\n Num of gauss before adding neg gauss = {gaussians._xyz.shape[0]}")
                    adding_neg_gauss_flag = True
                    neg_gaus_3d_means, blob_scales_tensor, blob_rotation = get_new_neg_points(gt_image, viewpoint_cam, means_2D, depths)
                    # Test
                    neg_gauss_mask = torch.rand(neg_gaus_3d_means.shape[0]) < 0.1 # 0.1 Getting only 10%
