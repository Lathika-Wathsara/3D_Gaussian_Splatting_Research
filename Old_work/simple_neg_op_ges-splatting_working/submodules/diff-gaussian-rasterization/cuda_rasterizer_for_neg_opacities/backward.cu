	...
	glm::vec4* dL_drot)
{
	...
	if (shs)	// Code by lathika - add &&(con_o.w > 0.0f) , only for positive gaussinas, mean will be affected by colors
	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float2 xy = collected_xy[j];	// Code by lathika removed const
			const float2 d = { xy.x - pixf.x, xy.y - pixf.y };	// Code by lathika removed const
			const float4 con_o = collected_conic_opacity[j];
			float power = -0.5f * (con_o.x * d.x * d.x + con_o.z * d.y * d.y) - con_o.y * d.x * d.y;	// Code by lathika removed const
			if (power > 0.0f)
				continue;

			const float G = exp(power);	// Code by lathika removed const

			//const float alpha = min(0.99f, con_o.w * G); // Code by lathika - commented
			// if (alpha < 1.0f / 255.0f)				   // Code by lathika - commented
			// 	continue;

			// Code by lathika
			float alpha = (con_o.w> 0.0f)? min(0.99f, con_o.w * G) : max(-0.99f, con_o.w * G);
			if ((alpha < 1.0f / 255.0f && con_o.w > 0.0f) || (alpha > -1.0f / 255.0f && con_o.w < 0.0f))
				continue;
