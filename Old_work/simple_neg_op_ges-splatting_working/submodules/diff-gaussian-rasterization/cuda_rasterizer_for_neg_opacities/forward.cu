	...
	uint32_t*  __restrict__ contrib_count)	// Code by lathika
{
	...
	{	
		...
		{	
			...
			// Code by lathika
			float alpha = (con_o.w > 0.0f) ? min(0.99f, con_o.w * exp(power)) : max(-0.99f, con_o.w * exp(power)); // handelling neg op
			if ((con_o.w>0.0f && alpha < 1.0f / 255.0f) || (con_o.w<0.0f && alpha > -1.0f / 255.0f))
				continue;
			...
			// float alpha = min(0.99f, con_o.w * exp(power));	// Code by lathika - Commented
