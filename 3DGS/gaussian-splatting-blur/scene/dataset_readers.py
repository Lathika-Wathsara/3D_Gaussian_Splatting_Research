...
# Code by lathika
import open3d as o3d    # install this if it's not installed
...
def downsample_voxel_grid(point_cloud: BasicPointCloud, voxel_size: float) -> BasicPointCloud:  
    ...
    """
    Typical Values
        Small, detailed object: Use a voxel size of 0.001 to 0.01 units (if the unit of measurement is meters).
        Medium-sized objects (e.g., furniture or vehicles): Use a voxel size of 0.01 to 0.05 units.
        Large-scale environments (e.g., buildings or outdoor scenes): Use a voxel size of 0.1 to 0.5 units.
    Examples of Voxel Sizes
        0.005 to 0.01: Preserves fine details for small objects or localized structures.
        0.02 to 0.05: Useful for general-purpose downsampling of medium-size scenes.
        0.1 or greater: Reduces point count significantly for large-scale scenes while preserving global structure.
    """
    ...
    #test
    print("initial point cloud size =",point_cloud.points.shape[0])
    print("After downsampled point cloud size = ", np.asarray(downsampled_cloud.points).shape[0])
