...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, log_to_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        image, viewspace_point_tensor, visibility_filter, neg_gauss_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["neg_gauss_filter"], render_pkg["radii"]
        ...
        with torch.no_grad():
            ...
            # Code by lathika
            if iteration%100==0:
                print(f"radii_len = {len(radii)}, total_wanted_points = {torch.sum(~neg_gauss_filter)} , unwanted_neg_points = {torch.sum(neg_gauss_filter)}")

            # Densification
            if iteration < 20000: #opt.densify_until_iter:  # Code by lathika (changed the value)
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if iteration > 1000 and iteration%200 ==0:
                        gaussians.prune_unwanted_neg_gauss(neg_gauss_filter)    # Code by lathika
