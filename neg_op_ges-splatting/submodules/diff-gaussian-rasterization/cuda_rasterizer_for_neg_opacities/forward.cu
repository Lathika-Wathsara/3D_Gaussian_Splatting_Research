...
renderCUDA(
	...
	int* __restrict__ unwanted_neg_gauss,	// Code by lathika
	...
	float* __restrict__ out_color)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (con_o.w < 0.0f)
			{
				float alpha = max(-0.99f, con_o.w * exp(power));
					...
					if (abs(pos_depth-neg_depth) > 2*(glm::sqrt(pos_var)+glm::sqrt(neg_var)))
					{
						...
						else
						{
							...
							for (int v = u+1; v < pointer+1; v++){
								...
								unwanted_neg_gauss[idx] = 1;	// 1 means, this gaussian should be considered
			...
			// Code by lathika
			unwanted_neg_gauss[collected_id[j]]=1;	// If a gaussian has a valid positive opacity, then add it to the filter
...
void FORWARD::render(
	...
	const float* colors,
	int* unwanted_neg_gauss,	// Code by lathika
	...
	float* out_color)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		...
		means2D,
		colors,
		unwanted_neg_gauss,	// Code by lathika
