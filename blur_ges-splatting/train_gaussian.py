...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, log_to_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Code by lathika
        active_blur = True
        blur_margine_iter = min(opt.iterations, opt.densify_until_iter)
        if iteration < blur_margine_iter//3 and active_blur:
            kernel_size = 5
            gt_image = conv_and_get_img(kernel_size, gt_image)
        elif iteration < 2 * blur_margine_iter//3 and active_blur:
            kernel_size = 3
            gt_image = conv_and_get_img(kernel_size, gt_image)
        else:
            kernel_size = 1
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    # Code by lathika
                    size_threshold = 20 * kernel_size if iteration > opt.opacity_reset_interval else None
                    gaussians.densify_and_prune(opt.densify_grad_threshold*kernel_size, opt.prune_opacity_threshold*kernel_size, scene.cameras_extent, size_threshold)
